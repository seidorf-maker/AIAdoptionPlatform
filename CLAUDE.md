...
- **`/pricing`** — a public, buyer-facing (HR/L&D lead, not Denise) pricing page: ROI framing reusing the `research/PRD.md` §1 market-sizing figures, three tiers mapped to the `size_tier` segmentation in `research/PRD.md` §4.3 (Starter <500 @ $12/participating-employee/mo, Growth 500–2,000 @ $8/mo, Enterprise 2,000–5,000+ @ custom), all explicitly priced per participating employee, not full headcount. No fake checkout — CTAs are plain `mailto:` links (placeholder address `hello@onramp.ai`, swap before real use) per the no-fake-purchase-flow rule in `research/viability-analysis.md`. Linked from `site-header.tsx` nav, not gated by `web/src/proxy.ts`.
- **`/about`** — public "Our Mission" page, same ungated pattern as `/pricing`, founder origin story with generic (non-real-employer) attribution, plus the same sourced market-pattern stat. Linked from nav as "Our Mission."
- **Market-sizing stats re-verified against primary sources (2026-07-28):** the original "67% use AI / 33% trained" framing and its "SAP UK"-cited "60% doesn't convert" assumption could not be traced to any locatable source via live web research, so both were replaced everywhere (`research/PRD.md` §1, `/pricing`, `/about`) with figures actually verified via Firecrawl: Gallup's 2025 workforce survey (44% integrating AI vs. 30% with formal guidelines) and a Melbourne Business School/University of Melbourne 47-country 2025 study (42% use employer-provided AI tools vs. 70% using free public tools). The $20,868-company TAM and $379.88/user/year LinkedIn Learning figure held up and were kept, now with real citable URLs. The $960K illustrative-spend figure became ~$800K, rederived transparently from the new sourced inputs (see `research/PRD.md` §1's new Sources list) rather than the unverifiable 60% assumption. `/pricing` and `/about` now show live citation links to the actual sources, not just internal doc footnotes.
- **i18n (2026-07-28):** `web/src/lib/i18n/dictionaries.ts` + `locale-context.tsx` — a lightweight client-side locale context (English/Spanish/French dictionaries, no path-prefix routing so `web/src/proxy.ts`'s exact-path auth checks stay untouched), persisted via an `onramp_locale` cookie read server-side in `layout.tsx` to avoid a language flash on load. Covers `/`, `/login`, `/pricing`, `/about`, the shared nav, and the footer — `/app`'s interactive tools (Prompt Coach, Competence Assessment, Community, Leaderboard) are still English-only. Switcher lives in `site-header.tsx`.
- **Community — now its own public page at `/community` (moved 2026-07-28):** `web/src/components/onramp/community.tsx`, Stack-Overflow-style — 3 seeded posts (prompts/agent-instructions by track) with answers, an "accepted answer" marker, upvotes, and a working post/answer composer. All client-side React state (`web/src/lib/onramp/community-posts.ts` is the seed data) — nothing persists across a reload, no backend yet. **Originally a tab inside `/app`; per founder direction it was pulled out into its own standalone section**: a public route `web/src/app/community/page.tsx` (ungated by `web/src/proxy.ts`, same as `/pricing`/`/about`), linked from `site-header.tsx` nav as "Community", and removed both from the `/app` tab bar and from the signed-out homepage locked-preview. The page resolves the signed-in user's name for post attribution if there is one, else a neutral "You" guest identity. The component still carries a `mode: "live" | "preview"` prop for shape-consistency with its siblings, but only `"live"` is now used.
- **Leaderboard/gamification tab (2026-07-28):** a fifth tab in `/app` (`web/src/components/onramp/leaderboard.tsx`) plus a "Team Leaderboards" add-on line on `/pricing` (+$2/participating-employee/mo). Two intentionally separate views: a business-unit leaderboard (always aggregate, never names an employee) and an opt-in individual leaderboard, off by default, that even when opted in only ever shows positive achievement (points/certifications/streak) — never a failed attempt. This is the founder-directed revision to the "no leaderboards" rule described in §7 below; read that section before touching this feature.
- **Leaderboard in the signed-out landing-page preview (2026-07-28):** the onramp components take a `mode: "live" | "preview"` prop, same pattern as Prompt Coach/Competence Assessment — in `preview` mode `Leaderboard` defaults to opted-in, so a signed-out visitor on `/` sees it filled with realistic content inside the same `LockedOverlay` (disabled fieldset + "🔒 Log in to try it" badge), not an empty state. The homepage locked-preview now covers Prompt Coach, Competence Assessment, and Leaderboard; **Community was removed from this preview** when it moved to its own public `/community` page (see above).
- **Color palette refresh (2026-07-28):** `web/src/app/globals.css` accent/success/review tokens (light + dark) were deepened/saturated for a warmer, more confident "Apple-esque, not techy" look per founder direction — see the in-file comments for the reasoning. Also added a `--success-foreground` token (mirroring the existing `--accent-foreground` light/dark flip) and fixed a real pre-existing dark-mode contrast bug: `learning-paths.tsx`'s "Added ✓" chip hardcoded `text-white` on `bg-success`, which is mathematically unreadable once success is the light, dark-mode-appropriate tone (a color can't simultaneously be light-enough to read as text against a near-black background and dark-enough for white text to read against it as a fill) — now uses the new themed `text-success-foreground` instead. **Superseded later the same day by the AWS-inspired navy/blue rebrand, then the green-accent switch — see below.**
- **New logo + green-primary accent (2026-07-28, latest):** the founder supplied a new OnRamp logo — a dark-navy rounded icon square containing a diagonal "ramp" bar and a green certification-check badge, plus a wordmark with "On" in the theme foreground and "Ramp" in green. Recreated as crisp SVG (not a binary asset): `web/src/app/icon.svg` (favicon) and `web/src/components/brand-logo.tsx` (`OnRampIcon` + `BrandLogo`, used in `site-header.tsx` in place of the old plain-text "OnRamp"). A reference copy lives at `brand/onramp-icon-2026.svg` (the older `brand/*.png` files are a **different, superseded** blue-chart logo — don't use them). The logo was adopted, and the palette briefly went **all-green** (primary `--accent` switched from blue to green to match). The founder then found that too monochrome and asked for **blue + green throughout** — so as of the latest revision: **primary `--accent` is blue again** (light `#0b5fae`, dark `#4da8ff`) driving buttons, links, eyebrows, and active tabs, while **green (`--success`, light `#147a45` / dark `#57cf8e`) is the brand + success color**: the logo "Ramp", the very-top header "Log in" button (kept green to tie with the logo — in-page CTAs are blue), success pills, "Added ✓" chips, and the ROI callout. Added a `--success-glow` token so the green header button gets the same hover-glow treatment as the blue ones. Gold `--review` remains the third pill tone. Backgrounds stay white (light) / cool navy (dark). Net: blue = primary action, green = brand/success, both visible on every page.
- **Aurora hero background (2026-07-28):** integrated React Bits' `Aurora` WebGL component (`web/src/components/aurora.tsx`, adapted from JSX to TypeScript) — an animated blue→green→navy aurora glow. Adds one dependency, **`ogl`** (`^1.0.11`, ships its own types). The landing page (`web/src/app/page.tsx`) hero was restructured into a **dark navy band** (`bg-[#0f1720]`, always dark regardless of theme so the glow reads well) with the aurora absolutely positioned behind (`pointer-events-none`) and the headline/subhead/Log-In button in white on top; the rest of `/` (previews, principles) stays on the normal themed background below. Colors are brand-matched (`colorStops={["#4da8ff","#57cf8e","#0F1720"]}`). Client-only (`"use client"`, requestAnimationFrame + WebGL2); the trivial upstream `Aurora.css` (width/height 100%) was inlined as an `h-full w-full` class rather than a separate global stylesheet.
- **SpecularButton (2026-07-28):** integrated React Bits' `SpecularButton` (`web/src/components/specular-button.tsx` + `specular-button.css`, adapted from JSX to TS) — a glassy WebGL button whose specular rim highlight steers toward the cursor (same `ogl` dep, no new install). Used as the landing **hero "Log In" CTA** on the dark aurora band (the effect needs a dark surface), replacing the old solid `PrimaryLink`. It renders a native `<button>`, so navigation is wired via `useRouter().push("/login")` in `onClick`. Brand-matched props: glassy fill (`tintOpacity 0.08`, `blur 4`), green specular line (`#7cff67`) over a blue base (`#4da8ff`), pill radius, `followMouse`. Its substantial CSS is a real stylesheet imported by the component (works from a component in the App Router — verified in the production build), unlike Aurora's inlined one-liner.
...
web/                          # Next.js frontend prototype — see web/README.md
  ...
  src/app/                     # App Router pages: / (landing+preview), login, app (protected demo), pricing, verify/[code], api/claude
  src/lib/supabase/              # browser + server Supabase client helpers
  src/lib/onramp/                # ported onramp-demo.html scenarios + Claude system prompts
  src/lib/mock-data.ts          # demo data backing /verify/[code] only (see web/README.md)
  src/components/                # shared UI primitives + components/onramp/ (Prompt Coach, Competence Assessment, etc.)
...
- **No visible failure states in front of peers — still absolutely non-negotiable.** A failed attempt or a `needs_review` result is never shown to anyone but the person who submitted it, full stop, regardless of anything below.
- **Public exposure is opt-in only (revised 2026-07-28, founder direction).** Two gamification surfaces now exist, both demoed in `web/src/app/app/`: (1) **team/business-unit leaderboards** — always aggregate, never name an individual employee, visible to managers as part of a paid add-on for leadership tracking; (2) **individual leaderboards** — opt-in only, off by default, and even when opted in they rank *positive achievement only* (certifications earned, points, streaks) — never failed attempts, never a `needs_review` status. Opting in to be ranked is not the same as consenting to have a bad result shown; that line doesn't move.
- **Low time cost.** She evaluates everything against "I don't have time to go down a rabbit hole."

**Brand voice:** mentor/guide archetype, not a tech-disruptor brand. Plain language over jargon ("let AI draft the first pass" not "leverage generative AI capabilities"). Permission-giving phrasing ("here's exactly what to use," "this is approved," "step by step"). Quietly celebratory at milestones — the opt-in gamification above is the one deliberate exception to "never gamified hype," and even there the tone stays warm/competitive-for-fun rather than cutthroat. Tagline: *"AI, for the rest of us."* **Visual direction (revised again 2026-07-28, later same day, founder-directed):** the earlier "warm neutrals, deliberately not cold blue" rule (see git history on this file for the exact prior wording) is now superseded — the founder explicitly chose to move toward a navy/blue + green palette inspired by the AWS Training & Certification page, plus AWS-style hover-brightening on buttons, over the documented tension that this reads closer to mainstream enterprise-tech than the original "warm mentor, not a tech-disruptor brand" positioning. This was surfaced and flagged before implementing, not silently changed — if this ever gets revisited, the original rationale was: Denise's core fear is that a tool will look/feel like "the tech team's thing," and blue/navy enterprise-SaaS styling is the visual language she associates with that. The new direction accepts that tradeoff deliberately. **Follow-up the same day:** a new OnRamp logo (navy icon square + green ramp-check + green "Ramp" wordmark) was adopted. The accent briefly went all-green to match, but the founder found that too monochrome and settled on **blue + green together**: blue `--accent` for primary actions (buttons/links/eyebrows/tabs), green for the brand (logo + top-nav "Log in" button) and success states. See `web/src/app/globals.css` for the implemented palette, `web/src/components/brand-logo.tsx` for the logo, and §3 above for the fuller note.
