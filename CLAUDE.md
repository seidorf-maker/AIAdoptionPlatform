...
- **Community and Leaderboard added to the signed-out landing-page preview (2026-07-28):** both components take a `mode: "live" | "preview"` prop, same pattern as Prompt Coach/Competence Assessment — in `preview` mode, `Community` opens its first seeded post by default and `Leaderboard` defaults to opted-in, so a signed-out visitor on `/` sees both filled with realistic content inside the same `LockedOverlay` (disabled fieldset + "🔒 Log in to try it" badge), not an empty state. This was a direct follow-up ask — prospective buyers need to see these without logging in.
