...
**Brand voice:** mentor/guide archetype, not a tech-disruptor brand. Plain language over jargon ("let AI draft the first pass" not "leverage generative AI capabilities"). Permission-giving phrasing ("here's exactly what to use," "this is approved," "step by step"). Quietly celebratory at milestones, never gamified hype. Tagline: *"AI, for the rest of us."* Visual direction (revised 2026-07-28): warm neutrals with a confident multi-tonal accent system — terracotta (primary), a real grass-green, and an ochre/gold, each used intentionally rather than just for status colors, aiming for "Apple-esque: clean, welcoming, a bit colorful" without drifting techy (see `web/src/app/globals.css` for the implemented palette) — still deliberately not the cold blue/purple "AI tech" gradient cliché.
